...
def train(model,
    ...
    loss_fc = nn.MSELoss()#.cuda() # biplav
    # loss_fc = nn.CrossEntropyLoss()#.cuda() # biplav
...
class DQN(nn.Module):
    def __init__(self):
        ...
        self.model = ResNet(self.state_size, self.n_actions)#.cuda() # biplav
        self.target_model = ResNet(self.state_size, self.n_actions)#.cuda() # biplav
    ...
    def choose_action(self, state):
        ...
        state = Variable(torch.from_numpy(state.astype(float))).float()#.cuda() # biplav
    ...
    def learn(self):
        ...
        state = Variable(torch.from_numpy(state.astype(int))).float()#.cuda() # biplav
        next_state = Variable(torch.from_numpy(
            next_state.astype(int))).float()#.cuda() # biplav
        ...
        q_target[batch_index, action] = torch.from_numpy(reward).float()\
            + self.gamma * torch.max(q_next, axis=1)[0].float() # biplav
