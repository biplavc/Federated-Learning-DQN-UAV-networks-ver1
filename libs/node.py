...
class Station(Node):
    ...
    def send_data(self):
        if self.channel.time > (self.time):
            ...
            self.channel.set_timer(self.channel.time if (self.channel.time) >
            ...
            self.total_pkt_time += self.frame_len # total_pkt_time incremented when send
        ...
        if (time <= self.time):
            ...
            if (time == self.send_time) and (time > 0):
                if (self.channel.collision) and (self.collision == 0):
                    ...
                    self.bin_back += 1
                    self.total_pkt_time -= self.frame_len # total_pkt_time decremented when wait
                    self.history[-1][-1] = 2
            ...
            if (time == self.time) and (time > 0):
                ...
                if (self.channel.collision) and (self.collision == 0):
                    ...
                    self.timeout.append(self.time + self.timeout_bar -
                                        self.ack_bar)
                    #reset backoff/dfis here
                    self.total_pkt_time -= self.frame_len # total_pkt_time incremented when no collision
